...
pub async fn insert_class_schedules(
    ...
    for offering_row in offerings.iter() {
        ...
        if let Err(err) = classes_resp { return Err(err); }
        ...
        for class in classes.iter() {
            ...
            if schedule_obj.is_none() { 
                error!("A Class Schedule is None for the course with offering id: {}", course_offering_id);
                continue; 
            }
            ...
            if class_section_obj.is_none() || class_number_obj.is_none() ||
                component_obj.is_none() || max_enrollment_obj.is_none() ||
                curr_enrollment_obj.is_none() || start_time_obj.is_none() ||
                end_time_obj.is_none() || days_code_obj.is_none()
            {
                error!("A Class Schedule of the course with offering id: {:?} has NULL value \
                for a non-null attribute.", course_offering_id);
                continue;
            }

            // The start/end time should be valid times.
            let start_time_str: String = start_time_obj.unwrap().split("T").nth(1).unwrap().to_owned();
            ...
            match start_time_str.parse::<NaiveTime>() {
                ...
                Err(_) => {
                    error!("Could not parse start time of class schedule. Start time: {:?}", start_time_str);
                    continue;
                },
            }
            ...
            match end_time_str.parse::<NaiveTime>() {
                ...
                Err(_) => {
                    error!("Could not parse end time of class schedule. End time: {:?}", start_time_str);
                    continue;
                },
            }
            ...
            match instructor_data {
                Some(instructors) => {
                    ...
                    for instructor in instructors {
                        let first_name_obj: Option<String> = instructor.instructor_first_name.flatten();
                        let last_name_obj: Option<String> = instructor.instructor_last_name.flatten();
            ...
            match result {
                Err(_) => {
                    return Err(format!("Could not execute insert_class_schedule query with values: \
                    class section({}), class number({}), component({}), \
                    start_time({}), end_time({}), days({}/{}/{}/{}/{}/{}/{}), \
                    instructor_name({:?}), location({:?}), course_offering_id({}) \
                    max_enrollment({}), curr_enrollment({})", class_section, class_number,
                    component, start_time, end_time, monday, tuesday, wednesday, thursday, friday, 
                    saturday, sunday, instructor_name, location, course_offering_id, max_enrollment, curr_enrollment));
                }
                _ => ()
            }
        }
